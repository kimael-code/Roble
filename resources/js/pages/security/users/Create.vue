...
watch(
  ...
  ([isOpenSheetPermissions, isOpenSheetRoles, isOpenSheetOUs]) => {
    if (!isOpenSheetRoles || !isOpenSheetPermissions || !isOpenSheetOUs) {
      ...
      router.visit(UserController.create(), {
        preserveScroll: true,
        preserveState: true,
      });
    }
...
function handleEmployeeSelection(employee: { [index: string]: any }) {
  form.email = employee.email || '';
  form.name = employee.email?.split('@')[0] || '';
  form.id_card = employee.id_card;
  form.names = employee.names;
  form.surnames = employee.surnames;
  form.position = employee.position;
  form.staff_type = employee.staff_type_name;
  form.ou_names.push(employee.org_unit_name);
                ...
                <Dialog v-model:open="openDialog">
                  ...
                  <DialogContent class="sm:max-w-[425px]">
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Busque y seleccione a un empleado. Si el usuario a
                        registrar no pertenece a la organización active el
                        suiche "Usuario Externo" para establecer los datos
                    ...
                    <div
                      ...
                      class="relative w-full max-w-sm items-center p-4"
                    >
                      <Input
                        id="search"
                        type="text"
                    ...
                    <ScrollArea
                      v-if="!form.is_external"
                      class="m-3 h-20 rounded-md border"
                    >
                      <div class="p-4">
                        <div v-for="(employee, i) in employees" :key="i">
                          <Label
                            :for="employee.id_card"
                          ...
                          >
                            ...
                            />
                            <span>{{
                              `${employee.nationality}-${employee.id_card} ${employee.names} ${employee.surnames}`
                            }}</span>
                          </Label>
                    ...
                    <div v-else class="grid gap-4">
                      <div class="grid grid-cols-4 items-center gap-4">
                        <Label for="id_card" class="is-required text-right">
                          Nro. de CI
                        </Label>
                        <Input
                          id="id_card"
                      ...
                      <div class="grid grid-cols-4 items-center gap-4">
                        <Label for="names" class="is-required text-right">
                          Nombres
                        </Label>
                        <Input
                          id="names"
                      ...
                      <div class="grid grid-cols-4 items-center gap-4">
                        <Label for="surnames" class="is-required text-right">
                          Apellidos
                        </Label>
                        <Input
                          id="surnames"
                    ...
                    <DialogFooter v-if="form.is_external">
                      <Button type="button" @click="handleExternalPerson">
                        Guardar
                      </Button>
                    </DialogFooter>
                  </DialogContent>
